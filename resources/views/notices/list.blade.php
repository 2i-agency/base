			...
			<div class="row">
				...
				@if ($notices_types->count())
					<div class="col-lg-3 col-md-2 col-sm-6 col-xs-12">
						<select name="type" class="form-control">
							<option value="all"{{ Request::get('type') == 'all' ? ' selected' : NULL }}>
								Любого типа
							</option>
							@foreach($notices_types as $notices_type)
								<option
									value="{{ $notices_type->id }}"
									{{ Request::get('type') == $notices_type->id ? 'selected' : NULL }}
								>
									{{ $notices_type->name }}
								</option>
							@endforeach
							<option value="none"{{ Request::get('type') == 'none' ? ' selected' : NULL }}>
								Без типа
							</option>
						</select>
					</div>
				...
				">
					<select name="is_read" class="form-control">
						<option value="all"{{ Request::get('is_read') == 'all' ? ' selected' : NULL }}>
							Прочитанные и непрочитанные
						</option>
						<option value="read"{{ Request::get('is_read') == 'read' ? ' selected' : NULL }}>
							Прочитанные
						</option>
						<option value="not_read"{{ Request::get('is_read') == 'not_read' ? ' selected' : NULL }}>
							Непрочитанные
						</option>
					</select>
				</div>
